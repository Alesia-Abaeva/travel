...
window.addEventListener('click', e => {
    ...
    if(!target.closest('.header__menu') && !target.closest('.header-menu__burger')){
...
const loginButtomMenu = document.querySelector('.buttom__logIn')
loginButtomMenu.addEventListener('click', function(){
    popUp.classList.toggle('hidden');
    popUp_container.classList.toggle('active-popup')
...
function clickOnButtonSingUP (){
    ...
    let valueInput = 'EMAIL:'+ ' ' + elemEmail + ' ;'+ ' ' +  ' ' + 'PASSWORD: ' +  ' ' + elemPass
    alert(valueInput)
}
...
window.addEventListener('click', e => {
    const target = e.target
    if(target.closest('.buttom_signUp')){
        clickOnButtonSingUP ()
    }
})
...
function changeSlide(direction){
    ...
    let mucg = mainSlide.style.transform = `translateX(${-((((activeSlideIndex)* width))) }px)` 
 
}
var circleBar = document.getElementsByClassName("scroll__bar_circle");
...
function showSlides(n){
    ...
    // let mucg = mainSlide.style.transform = `translateX(${-((((activeSlideIndex)* width))) }px)`
    if(windowInnerWidth >= 392){
        mucg = mainSlide.style.transform = `translateX(${-((((activeSlideIndex)* width)+600)-860) }px)`

    } else   mucg = mainSlide.style.transform = `translateX(${-((((activeSlideIndex)* width))) }px)`
    

    for (let i = 0; i < circleBar.length; i++) {
        circleBar[i].className = circleBar[i].className.replace(" active_circle", "");
    }
